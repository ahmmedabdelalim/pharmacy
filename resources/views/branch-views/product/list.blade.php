                    ...
                    <div class="card-header" style="padding-right: 0!important;">
                        <div class="container row">
                            ...
                            <div class="col-md-6 mb-3 mb-lg-0">
                                <a href="{{route('branch.product.add-new')}}" class="btn btn-primary float-right"><i
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="d-block font-size-sm text-body">
                                             <a href="{{route('branch.product.view',[$product['id']])}}">
                                    ...
                                    <td>
                                        <div style="height: 100px; width: 100px; overflow-x: hidden;overflow-y: hidden">
                                            <img
                                                src="{{asset('public/storage/app/public/product')}}/{{$product['image']}}"
                                                style="width: 100px"
                                    ...
                                    <td>
                                        @if($product['status']==1)
                                            <div style="padding: 10px;border: 1px solid;cursor: pointer"
                                                 onclick="location.href='{{route('branch.product.status',[$product['id'],0])}}'">
                                                <span
                                                    class="legend-indicator bg-success"></span>{{\App\CentralLogics\translate('active')}}
                                            </div>
                                        @else
                                            <div style="padding: 10px;border: 1px solid;cursor: pointer"
                                                 onclick="location.href='{{route('branch.product.status',[$product['id'],1])}}'">
                                                <span
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                <a class="dropdown-item"
                                                   href="{{route('branch.product.edit',[$product['id']])}}">{{\App\CentralLogics\translate('edit')}}</a>
...
@push('script_2')
    <script>
        $('#search-form').on('submit', function () {
            ...
            $.post({
                url: '{{route('branch.product.search')}}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
    ...
    <script>
        function daily_needs(id, status) {
            ...
            $.ajax({
                url: "{{route('branch.product.daily-needs')}}",
                method: 'POST',
                data: {
                    id: id,
                    status: status
